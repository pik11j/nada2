...
function setup(){
  ...
  suelo = createSprite(width/2,height-70,width,125);
  ...
  perdiste = createSprite(width/2,height-250,width,125);
  perdiste.addImage("hola",loser);
 
  flecha = createSprite(width/2,height-200,width,125);
...
function draw(){
  ...
  text("putuasion:" + score,width/2,height-700);
  ...
  if(touches.length > 0 || keyDown("space") && trex.y > 580){
    trex.velocityY = -10;
    jumpSound.play();
    //touches = [];
  }
...
function famili_captus(){
  if(frameCount % 150 === 0){
    var captus = createSprite(width/1,height-84,width,125);
    captus.velocityX = -2;
...
function nubes(){
  ...
  if(frameCount % 60 === 0){
    ...
    nube.addImage(nubep);
    nube.y = Math.round(random(280,height-400));
    nube.velocityX = -3;
    nube.scale = 0.5;
